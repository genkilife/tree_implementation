...
int main(){
	BplusTree* bpPtr = new BplusTree();
	bpPtr->printTreeMethod();
	...
	*/
	...
	for(unsigned int idx=0; idx < valSize; idx++){
		if(bpPtr->insert(vals[idx]) != SUCCESS){
			assert(false);
		}
		if((idx % SAMPLE_PERIOD) == 0){
			...
			if(idx > 0){
				...
				for(unsigned iidx=0 ; iidx < SAMPLE_PERIOD; iidx++){
					if(bpPtr->search(shuffledKeys[iidx]) != SUCCESS){
						assert(false);
					}
				}
	...
	*/
	fstream fs;
	fs.open("result/bplus_tree_skewed_insert_timestamp.txt", fstream::out);
	for(unsigned int idx=0; idx < valSize/SAMPLE_PERIOD; idx++){
		fs << idx*SAMPLE_PERIOD << " " << msecInsert[idx] << endl;
	}
	fs.close();
	cout<<"Finish bplus tree insertion "<<valSize <<" elements"<<endl;

	fs.open("result/bplus_tree_skewed_search_timestamp.txt", fstream::out);
	for(unsigned int idx=0; idx < valSize/SAMPLE_PERIOD; idx++){
